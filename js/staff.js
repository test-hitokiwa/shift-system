...
async function renderDateCalendar() {
    ...
    try {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            html += `<div class="${classNames.join(' ')}" onclick="${!isPast ? `toggleDateSelection('${dateStr}')` : ''}">`;
...
function generateCalendar(year, month, shifts, requests) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        html += `<div class="calendar-day ${hasData ? 'has-shift' : 'clickable-empty'}" onclick="${!hasData ? `openStaffQuickCreate('${dateStr}')` : ''}" style="${!hasData ? 'cursor: pointer;' : ''}">`;
        ...
        if (dayRequests.length > 0) {
            dayRequests.forEach(request => {
                ...
                // 希望シフトは未承認・承認済み両方とも黄色で表示
                html += `<div class="shift-info request-pending" style="cursor: pointer;" onclick="event.stopPropagation(); showRequestDetail('${request.id}')" title="${request.status === 'pending' ? '未承認' : '承認済み'}">${timeSlot}</div>`;
            });
        }
