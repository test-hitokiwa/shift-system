...
async function loadShifts() {
    try {
        const { shifts } = await getCachedData();
...
async function loadCalendar() {
    try {
        ...
        // 未承認の希望シフトをフィルター
        const pendingRequests = requestsResult.data.filter(req => 
            req.status === 'pending' &&
            req.date.startsWith(calendarMonth)
        );
        
        // 承認済みの希望シフトと確定シフトを結合
        const approvedRequests = requestsResult.data.filter(req => 
            req.status === 'approved' &&
            req.date.startsWith(calendarMonth)
        );
        
        const confirmedShifts = shiftsResult.data.filter(shift => 
            shift.is_confirmed &&
...
function generateRequestCalendar(year, month, requests) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        html += `<div class="calendar-day ${dayRequests.length > 0 ? 'has-shift' : ''}">`;
...
function generateApprovedCalendar(year, month, approvedRequests, confirmedShifts) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        html += `<div class="calendar-day ${hasData ? 'has-shift' : 'clickable-empty'}" onclick="${!hasData ? `openGeneralQuickCreate('${dateStr}')` : ''}" style="${!hasData ? 'cursor: pointer;' : ''}">`;
        html += `<div class="day-number">${day}</div>`;
...
async function loadManagementCalendar() {
    try {
        ...
        // フィルター
        const staffRequests = requestsResult.data.filter(r => 
            r.user_id === staffId && r.date.startsWith(month)
        );
        const staffShifts = shiftsResult.data.filter(s => 
...
function generateManagementCalendar(year, month, requests, shifts) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        html += `<div class="calendar-day ${hasData ? 'has-shift' : 'clickable-empty'}" onclick="${!hasData ? `openQuickCreateForDate('${dateStr}')` : ''}" style="${!hasData ? 'cursor: pointer;' : ''}">`;
        html += `<div class="day-number">${day}</div>`;
